...
partial class ProcessesForm : Form {
	...
	public ProcessesForm() {
		...
		// Initialize dumper type menu
		for (var dumperTypeValue = DumperType.Normal; dumperTypeValue <= DumperType.AdvancedAntiAntiDump; dumperTypeValue++) {
			var item = new ToolStripMenuItem(GetDumperTypeDisplayName(dumperTypeValue));
			var currentDumperType = dumperTypeValue;
	...
	void SwitchDumperType(DumperType dumperType) {
		string name = GetDumperTypeDisplayName(dumperType);
		foreach (ToolStripMenuItem item in mnuDumperType.DropDownItems)
			item.Checked = item.Text == name;
		this.dumperType.Value = dumperType;
		
		// Update UI feedback
		title.Annotations["DUMPER"] = dumperType switch {
			DumperType.Normal => null,
			DumperType.AntiAntiDump => "AntiAntiDump",
			DumperType.AdvancedAntiAntiDump => "Advanced",
			_ => null
		};
		Text = title.Compose(true);
	}
	...
	static string GetDumperTypeDisplayName(DumperType type) {
		return type switch {
			DumperType.Normal => "Normal",
			DumperType.AntiAntiDump => "AntiAntiDump",
			DumperType.AdvancedAntiAntiDump => "Advanced (Reactor)",
			_ => type.ToString()
		};
	}
}